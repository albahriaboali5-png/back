...
from .serializers import (
    ...
    AdBannerSerializer, UserDeviceSerializer, SystemSettingSerializer
)
...
from .models import (
    ...
    AIChatMessage, AdBanner, UserDevice, SystemSetting
)
import random
import string
...
class UserDeviceViewSet(viewsets.ModelViewSet):
    ...
    def set_primary(self, request, pk=None):
        ...
        UserDevice.objects.filter(user=request.user).update(is_primary=False)
        device.is_primary = True
        device.save()
        return Response({'status': 'primary device updated'})

class SystemStatusView(APIView):
    permission_classes = [permissions.AllowAny]
    def get(self, request):
        return Response({
            'maintenance_mode': SystemSetting.is_enabled('MAINTENANCE_MODE'),
            'registration_enabled': SystemSetting.is_enabled('REGISTRATION_ENABLED', True),
...
class RegisterView(APIView):
    def post(self, request):
        ...
        if SystemSetting.get_setting('REGISTRATION_ENABLED', 'true').lower() == 'false':
            return Response({'error': 'تسجيل الحسابات الجديدة موقوف حالياً من قبل الإدارة.'}, status=status.HTTP_403_FORBIDDEN)
        ...
        try:
            with transaction.atomic():
                ...
                # Create UserDevice (First device is primary)
                if device_id:
                    UserDevice.objects.create(user=user, device_id=device_id, device_name=device_name, is_primary=True, is_verified=True)
...
class LoginView(APIView):
    def post(self, request):
        ...
        if SystemSetting.is_enabled('MAINTENANCE_MODE'):
            # Allow staff to bypass maintenance
            user_check = User.objects.filter(username=email).first()
            if not user_check or not user_check.is_staff:
                return Response({
                    'status': 'maintenance',
                    'error': 'النظام في وضع الصيانة حالياً. يرجى المحاولة لاحقاً.'
        ...
        if user is not None:
            ...
            if device_id:
                device, created = UserDevice.objects.get_or_create(
                    user=user, 
                    device_id=device_id,
                    defaults={'device_name': device_name, 'is_verified': False}
                )
                
                if not device.is_verified and not user.is_staff:
                    # Generate 6-digit verification code
                    code = ''.join(random.choices(string.digits, k=6))
                    device.verification_code = code
                    device.verification_code_created_at = timezone.now()
                    device.save()

                    # Find primary device to send notification to
                    primary_device = UserDevice.objects.filter(user=user, is_primary=True).first()
                    msg = f"محاولة دخول من جهاز جديد ({device_name}). رمز التحقق هو: {code}"
                    
                    Notification.objects.create(
                        receiver=user,
                        title="تأكيد جهاز جديد",
                        message=msg
                    )
                    
                    # Also try to send FCM if token exists
                    send_fcm_notification(user, "تأكيد جهاز جديد", msg)

                    return Response({
                        'status': 'verification_required',
                        'message': 'دخول من جهاز جديد. تم إرسال رمز التحقق إلى جهازك الرئيسي.',
                        'device_id': device_id
...
class VerifyDeviceView(APIView):
    permission_classes = [permissions.AllowAny] # Allow verifying before token is active

    def post(self, request):
        email = request.data.get('email')
        password = request.data.get('password') # Re-verify credentials for safety
        device_id = request.data.get('device_id')
        code = request.data.get('code')

        if not all([email, password, device_id, code]):
        ...
        try:
            ...
            if device.verification_code == code:
                ...
                if timezone.now() - device.verification_code_created_at > timedelta(minutes=10):
                     return Response({'error': 'انتهت صلاحية الرمز.'}, status=status.HTTP_400_BAD_REQUEST)

                device.is_verified = True
                device.verification_code = None # Clear after use
                device.save()
                
                # Success! Now log them in normally
                token, created = Token.objects.get_or_create(user=user)
                profile = getattr(user, 'userprofile', None)
                role = profile.role if profile else 'PATIENT'
            ...
            return Response({'error': 'الجهاز غير موجود.'}, status=status.HTTP_404_NOT_FOUND)
